...
describe("DictionaryTestHelper", () => {
  ...
  describe("verifyDictionaryItem", () => {
    it("should verify an existing dictionary item", async () => {
      ...
      const exists = await DictionaryTestHelper.verifyDictionaryItem(
        builder.getId()
      );
  ...
  describe("getDictionaryItem", () => {
    it("should get dictionary item by id", async () => {
      ...
      const item = await DictionaryTestHelper.getDictionaryItem(
        builder.getId()
      );
      ...
      expect(item.translations[0].translation).toBe(
    ...
    it("should throw for non-existent dictionary item", async () => {
      await expect(
        DictionaryTestHelper.getDictionaryItem(BLANK_UUID)
      ).rejects.toThrow();
    });
  });
