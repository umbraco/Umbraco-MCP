...
describe("MediaTestHelper", () => {
  ...
  it("normaliseIds should blank out id for single and array", async () => {
    ...
    const normSingle = MediaTestHelper.normaliseIds(
      item
    ) as MediaTreeItemResponseModel;
    expect(normSingle.id).toBe(BLANK_UUID);
    const normArray = MediaTestHelper.normaliseIds([
      item,
    ]) as MediaTreeItemResponseModel[];
  ...
  it("findMediaInRecycleBin should find a media moved to the recycle bin", async () => {
    ...
    const foundNormal = await MediaTestHelper.findMedia(
      TEST_RECYCLE_BIN_MEDIA_NAME
    );
    expect(foundNormal).toBeUndefined();
    // Should be found in recycle bin
    const foundRecycleBin = await MediaTestHelper.findMediaInRecycleBin(
      TEST_RECYCLE_BIN_MEDIA_NAME
    );
  ...
  it("findMediaInRecycleBin should return undefined for non-existent media", async () => {
    const found = await MediaTestHelper.findMediaInRecycleBin(
      "NonExistentRecycleBinMedia"
    );
    expect(found).toBeUndefined();
  });
  ...
  it("emptyRecycleBin should remove all media from the recycle bin", async () => {
    ...
    // Should be found in recycle bin
    let found = await MediaTestHelper.findMediaInRecycleBin(
    ...
    found = await MediaTestHelper.findMediaInRecycleBin(
      "_Test EmptyRecycleBin"
    );
    expect(found).toBeUndefined();
  });
  ...
  it("restoreFromRecycleBin should restore a media item from recycle bin", async () => {
    ...
    let foundInRecycleBin = await MediaTestHelper.findMediaInRecycleBin(
      TEST_RESTORE_MEDIA_NAME
    );
    ...
    let foundInNormalTree = await MediaTestHelper.findMedia(
      TEST_RESTORE_MEDIA_NAME
    );
    ...
    foundInRecycleBin = await MediaTestHelper.findMediaInRecycleBin(
      TEST_RESTORE_MEDIA_NAME
    );
