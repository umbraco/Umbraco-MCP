...
describe("DocumentBuilder", () => {
  ...
  it("publish should throw if called before create", async () => {
    const builder = new DocumentBuilder()
      .withName("_Test PublishBuilder Error")
      .withRootDocumentType();
    await expect(builder.publish()).rejects.toThrow(
      /No document has been created yet/
    );
  });
  ...
  it("should update a document", async () => {
    ...
    const item = builder.getCreatedItem();
    expect(item).toBeDefined();
    expect(DocumentTestHelper.getNameFromItem(item)).toBe(
      TEST_UPDATED_DOCUMENT_NAME
    );
