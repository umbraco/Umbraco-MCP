...
export class DocumentTypeBuilder {
  ...
  withComposition(
    contentTypeId: string,
    compositionType: CompositionTypeModel = CompositionTypeModel.Composition
  ): DocumentTypeBuilder {
  ...
  async create(): Promise<DocumentTypeBuilder> {
    ...
    if (!createdItem) {
      throw new Error(
        `Failed to find created document type with name: ${name}`
      );
    }
  ...
  async update(): Promise<DocumentTypeBuilder> {
    ...
    const updatedItem = await DocumentTypeTestHelper.findDocumentType(
      this.model.name
    );
    if (!updatedItem) {
      throw new Error(
        `Failed to find updated document type with name: ${this.model.name}`
      );
    }
