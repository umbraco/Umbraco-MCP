...
const GetDocumentBlueprintChildrenResource = CreateUmbracoTemplateResource(
  ...
  new ResourceTemplate("umbraco://document-blueprint/children?parentId={parentId}&skip={skip}&take={take}&foldersOnly={foldersOnly}", {
    list: undefined,
    complete: {
  ...
  async (uri, variables) => {
    try {
      ...
      const params = getTreeDocumentBlueprintChildrenQueryParams.parse(variables);
