...
describe('DataTypeFolderBuilder', () => {
  ...
  describe('construction', () => {
    it('should create a builder with default values', () => {
      const builder = new DataTypeFolderBuilder(TEST_FOLDER_NAME);
  ...
  describe('builder methods', () => {
    let builder: DataTypeFolderBuilder;

    beforeEach(() => {
      builder = new DataTypeFolderBuilder(TEST_FOLDER_NAME);
    });
  ...
  describe('creation and retrieval', () => {
    ...
    it('should create a folder with parent', async () => {
      // Create parent first
      const parentBuilder = await new DataTypeFolderBuilder(TEST_PARENT_FOLDER_NAME)
        .create();
    ...
    it('should handle creation failure', async () => {
      ...
      const builder1 = new DataTypeFolderBuilder(TEST_FOLDER_NAME);
      await builder1.create();

      const builder2 = new DataTypeFolderBuilder(TEST_FOLDER_NAME);
      await expect(builder2.create()).rejects.toThrow();
    });

    it('should verify created folder exists', async () => {
      const builder = await new DataTypeFolderBuilder(TEST_FOLDER_NAME)
        .create();
  ...
  describe('error handling', () => {
    ...
    it('should handle creation with missing name', async () => {
      ...
      const builder = new DataTypeFolderBuilder();
      await expect(builder.create()).rejects.toThrow();
    });
