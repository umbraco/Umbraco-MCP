...
if (!client_secret && client_id !== "umbraco-swagger")
    throw new Error("Missing required environment variable: UMBRACO_CLIENT_SECRET");
...
const fetchAccessToken = async () => {
    const response = await Axios.post(`${baseURL}${tokenPath}`, {
        client_id,
...
UmbracoAxios.interceptors.response.use((response) => response, // Pass through successful responses
(error) => {
    if (error.response) {
...
export const UmbracoManagementClient = (config, options) => {
